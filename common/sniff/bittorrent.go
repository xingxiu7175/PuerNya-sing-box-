...
func BittorrentTCPMessage(ctx context.Context, reader io.Reader, sniffdata chan SniffData, wg *sync.WaitGroup) {
	var data SniffData
	defer func() {
		sniffdata <- data
		wg.Done()
...
func BittorrentUDPMessage(ctx context.Context, packet []byte, sniffdata chan SniffData, wg *sync.WaitGroup) {
	var data SniffData
	defer func() {
		sniffdata <- data
		wg.Done()
